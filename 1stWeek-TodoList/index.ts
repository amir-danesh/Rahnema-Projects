...
class Card {
    ...
    constructor(subject: string, deadLine: Date | null = null, labels: string[] = [], status: string = "todo"){
    ...
    showSingleCardDetails = () => {
        ...
        const deadLineText = `\nDeadline:   ${this.deadLine ? this.deadLine.toLocaleDateString() : "-"}`;
        const labelsText = `\nLabels:     ${this.labels.length > 0 ? this.labels.reduce((prev,curr) => prev + curr + ", ", "").slice(0,-2) : "-"}`;
        const statusText = `\nStatus:     ${this.status}`;
        const startDateText = `\nStart Date: ${this.startDate ? this.startDate.toLocaleDateString(): "-"}`;
        const endDateText = `\nEnd Date:   ${this.endDate ? this.endDate.toLocaleDateString(): "-"}`;

        const fullText = subjectText + deadLineText + labelsText + statusText + startDateText + endDateText + "\n";
    ...
    showSingleCardAbstract = (number: number | null = null) => {
        ...
        fullText += `${this.subject.slice(0,10)} ${this.subject.length > 10 ? "...   " : ""}`;
        fullText += `${this.deadLine ? this.deadLine.toLocaleDateString() + "   " : "No-Deadline   "}`;
    ...
    addLabel = (labelName: string) => {
        if(!labelTypes.includes(labelName.toLowerCase())){
            console.log(`!!!! Please provide a valid label, ${labelName} is not valid`);
            return
        }
    ...
    removeLabel = (labelName: string) => {
        ...
        this.labels = this.labels.filter((label) => label !== labelName.toLowerCase());
...
const createTask = (commandArray: string[]) => {
    ...
    const deadLine: Date | null = commandArray[2] && commandArray[2] !== "null" ? convertToDateObject(commandArray[2]) : null;
    const labels: string[] = commandArray[3] ? divideStringToArray(commandArray[3], ",") : [];
...
const showCommandsBeautify = () => {
    return Object.keys(commands).reduce((prev, curr, index) => prev + `\n${index + 1}-  ${curr}\n  Ex: ${commands[curr]}\n`.replace(/^ +/gm, ''), "" );
}

const showAllTasks = () => {
    tasks.map((x,i: number) => console.log("\n",x.showSingleCardAbstract(i + 1)))
}
...
const filterBySubject = (filterStr: string) => {
    const filteredTasks = tasks.filter((task) => task.subject.includes(filterStr))
...
// TEST DATA
const a = new Card("first card", convertToDateObject("10-07-1999"), ["yellow", "red","green"], "doing");
const b = new Card("taskName1", convertToDateObject("null"), ["Yellow","Red","Green"], "doing");
const c = new Card("taskName2", convertToDateObject("01-01-2020"), ["Yellow","Blue"], "todo");
const d = new Card("taskName3", convertToDateObject("null"), ["Yellow"], "done");
